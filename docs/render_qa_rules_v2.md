...
one-shot 자막이 beat당 최대 4개를 넘거나, 여러 chunk 중 최소 7자 미만 문구가 있으면 렌더 금지
끝난 문장 뒤에 다음 문장 조각을 같은 caption chunk로 붙이면 렌더 금지
caption_chunks를 합친 내용이 음성 전문과 다르거나 beat 시간을 연속으로 덮지 않으면 렌더 금지
포인트 pop의 절대 start_sec가 해당 chunk의 강조 단어 발화 예상 시점보다 0.20초 이상 빠르거나 0.45초 이상 늦으면 렌더 금지
포인트 pop이 420ms 동안 한 번 재생되지 않으면 렌더 금지
