...
- 핵심어 pop은 사진 전환과 동시에 시작하지 않습니다. `calm_dissolve 380ms`가 끝난 뒤
  숨 고르기를 포함해 chunk 시작 `520~650ms` 후에 시작하고, 기본값은 520ms입니다.
  pop은 420ms 동안 한 번만 올라왔다가 제자리로 돌아옵니다.
...
`y=220~1470`을 벗어나거나 3줄 이상이면 실패합니다. 핵심어의 유효 지연이 520~650ms
밖이거나 pop 길이가 420ms와 다르면 `CAPTION_ACCENT_TIMING_INVALID`로 실패합니다.
