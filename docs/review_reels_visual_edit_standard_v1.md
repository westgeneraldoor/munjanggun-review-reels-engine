...
- 첫 3컷 훅은 TTS 전 기획 길이도 최소 3.0초여야 합니다. 실측 발화 alignment 뒤
  세 shot을 각각 1.0초 이상 배치할 수 없으면 음수·역전 구간으로 클램프하지 않고
  `HOOK_ALIGNMENT_INFEASIBLE`로 명시 중단합니다.
...
- 자막 chunk 진입 pop은 그 chunk의 **실측 발화 시작**에서 220ms 동안 한 번만 재생합니다.
  CSS 실제 경과시간이 아니라 영상 시간으로 진행해 HTML 재생·스크럽·MP4 프레임에서
  같은 시각에 같은 위치를 보여야 합니다.
- 핵심어 pop은 chunk 시작의 고정 지연값으로 실행하지 않습니다. `caption_accent.start_sec`를
  **강조 단어의 실제 발화 예상 시점**에 결속하며, 해당 단어 위치를 글자 수로 산정한 시점보다
  0.20초 이상 먼저 튀면 실패합니다. pop은 240ms 동안 한 번만 올라왔다가 제자리로 돌아옵니다.
...
- renderer 확정값은 `calm_dissolve 520ms`, `calm_slide 600ms`,
  `soft_page_turn 640ms`입니다.
- 위 시간값은 엔진 상수 한 곳을 HTML 템플릿에 주입해 QA와 실제 화면이 같은 값을
  사용합니다. 현재 장면에서 보이는 레이어가 전환을 소유합니다. 일반 사진 장면은
  `mainAsset`, 리뷰 증거는 `reviewCard`, 제품 카드는 `productCard`가 전환되며 배경 사진을
  다시 보이게 해 카드를 침범하면 실패입니다. 영상 시간으로 JS가 직접 제어하는 opacity와
  filter에는 CSS 벽시계 transition을 중첩하지 않습니다.
- `calm_dissolve`는 520ms 동안 투명도만 전환합니다. 이전 사진은 마지막 카메라 위치를
  유지하며 dissolve가 별도의 확대·축소·이동 transform을 덮어쓰지 않습니다.
- `calm_slide`는 600ms 동안 새 사진을 한 방향으로 reveal하며 전체 영상에서 최대 2회만
  사용합니다. `soft_page_turn`은 640ms의 얕은 대각 reveal로 이야기의 핵심 전환에 최대
  1회만 사용합니다. 두 효과 모두 사진의 기존 calm transform을 덮어쓰지 않습니다.
- 세 전환은 CSS 실제 경과시간에 맡기지 않고 shot 시작부터의 영상 시간으로 진행률을
  계산합니다. 따라서 렌더 장비나 프레임 캡처 속도가 달라도 효과 길이가 줄어들지 않습니다.
...
`caption_accent.start_sec`가 있어야 하고 pop 길이가 240ms와 다르면
