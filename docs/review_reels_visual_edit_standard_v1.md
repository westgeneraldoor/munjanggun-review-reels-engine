...
- 첫 3컷은 각각 하나의 완결된 음성·자막 주장과 시작·종료 시각을 정확히 같이 씁니다. 각 shot의 `meaning_match_source`는 해당 `asset_evidence`와 실제 `narration_fragment`를 함께 결속합니다. 사진 경계가 자막 주장 중간을 가르거나 시공 전 화면에 설치 후 결과를 말하면 하드 실패입니다.
