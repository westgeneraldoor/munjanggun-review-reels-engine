...
완성→이전→완성 3컷 훅은 각 shot을 최소 1.0초 확보할 수 있도록 TTS 전 기획 길이도
3.0초 이상이어야 합니다. 실측 alignment로 이 배치가 불가능하면 시간을 음수로
클램프하거나 억지 배치하지 않고 `HOOK_ALIGNMENT_INFEASIBLE`로 중단합니다.
...
완성된 one-shot planning은 `writer_brief.story_spine`에 문제→행동→변화→증명→CTA를
순서대로 기록하고 각 단계에 원문·사진 근거와 실제 beat/caption 결속을 남깁니다. CTA는
첫 문제 단계를 회수해야 합니다. 작가·총괄 PD가 현재 표준 script를 승인한 뒤
`script_review`에 승인자·근거·script SHA-256·단계별 확인을 기록하며, 이 해시가 현재
`*_script.md`와 다르면 TTS를 호출하지 않습니다.
