...
## 리뷰 릴스 one-shot 예외 (현재 권위)

사용자가 리뷰 번호를 정한 뒤 `사진 다 넣었어. HTML까지 가자` 또는 같은 의미를 명시하면 `docs/review_reels_one_shot_contract_v1.md`를 따른다. 이는 사진검수부터 HTML 프리뷰까지의 일괄 승인이고, MP4 렌더 승인은 아니다. 이 경우 작가/PD/사진/QA의 내부 선택을 되묻지 않는다.
