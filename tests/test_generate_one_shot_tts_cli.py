...
class OneShotTTSCLIAuthoringGateTests(unittest.TestCase):
    ...
    def test_layout_failure_stops_before_the_tts_writer(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            package = Path(temp_dir)
            planning = package / "planning.json"
            edit = package / "edit.json"
            ...
            with (
                patch(
                    "scripts.generate_one_shot_tts.validate_review_reels_one_shot_authoring",
                    return_value={"ok": True, "issues": []},
                ),
                patch(
                    "scripts.generate_one_shot_tts.run_layout_precheck",
                    return_value={
                        "status": "fail",
                        "issues": [{"code": "CAPTION_DOM_LINE_COUNT_EXCESSIVE"}],
                    },
                ),
                patch("scripts.generate_one_shot_tts.generate_one_shot_tts") as writer,
            ):
                return_code = cli.main(
                    [
                        "--package", str(package),
